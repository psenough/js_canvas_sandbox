...
function drawCanvas() {
	...
	function drawSquares(timer) {
		...
		for (let j=0; j<scheduled_squares.length; j++) {
			let timed = timer-scheduled_squares[j]['inittime'];
			let playtime = 9000;
			if ((timed > 0) && (timed < playtime)) {
				let initimg = scheduled_squares[j]['initimg'];
				let initx = scheduled_squares[j]['initx'];
				let inity = scheduled_squares[j]['inity'];
				let niter = scheduled_squares[j]['niter'];
				let speed = scheduled_squares[j]['speed'];
				let width = scheduled_squares[j]['width'];
				let rotate = timed*0.1;
				ctx.save();
				var im = initimg+niter; //img_ref.length;
				let i = im;
				//for (var i=initimg; i<im; i++) {
					...
					//if (radius > 0) {
						...
						ctx.beginPath();
						ctx.rect(-width, -width,  width*2, width*2);
						//ctx.arc(initx, inity, radius, 0, 2 * Math.PI);
						ctx.restore();
						ctx.clip();
						...
						//ctx.drawImage(img_ref[i],0,0,w,h);
						var sx = 0.0;//35.0+Math.sin(timer*0.0001)*35.0;
						var sy = 0.0;//35.0-Math.cos(timer*0.0001)*35.0;
						// fader
						var fader = 0.0;
						// fadein
						fader = 1.0 - timed/playtime*10.0;
						if (fader < 0.0) fader = 0.0;
						// fadeout
						if (timed > playtime*0.9) fader = ((timed - playtime*0.9)/(playtime*0.1));
						// draw fader layer
