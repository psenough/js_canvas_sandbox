...
function initShaderProgramQuad() {
	...
	var fragCode3 = `
		...
		void main()
		{
			...
			float div = 1.9;
			if (length(color0)/div < 0.53) { color0 = texture2D(u_texture1, v_texCoord); }
			else if (length(color0)/div < 0.58) { color0 = texture2D(u_texture2, v_texCoord); }
			else if (length(color0)/div < 0.62) { color0 = texture2D(u_texture6, v_texCoord); }
			else if (length(color0)/div < 0.7) { color0 = texture2D(u_texture4, v_texCoord); }
			else if (length(color0)/div < 0.75) { color0 = texture2D(u_texture7, v_texCoord); }
			else if (length(color0)/div < 0.8) { color0 = texture2D(u_texture3, v_texCoord); }
			else if (length(color0)/div < 0.85) { color0 = texture2D(u_texture5, v_texCoord); }
			else if (length(color0)/div < 0.9) { color0 = texture2D(u_texture6, v_texCoord); }
			else if (length(color0)/div < 0.95) { color0 = texture2D(u_texture7, v_texCoord); }
			else { color0 = texture2D(u_texture8, v_texCoord); }
			
			
			//color0 += texture2D(u_texture8, v_texCoord);
			color0 = clamp(color0,0.0,1.0);
	...
	var fragCode4 = `
		...
		vec4 cHashA4 = vec4 (0., 1., 57., 58.);
		vec3 cHashA3 = vec3 (1., 57., 113.);
		...
		void main()
		{
			...
			iTime = u_timer*0.0008;
				
			//uv.x *= iResolution.x / iResolution.y;
			tCur = iTime;
			vec2 p = uv;
			for (int i = 0; i < 12; i ++) p -= FlowField (p) * 0.05;

			vec3 fragColor = Fbm2 (5. * p + vec2 (-0.2 * tCur, 1.*sin(iTime*0.15)*cos(iTime*0.25))) * vec3 (0.75, 0.75, 1.);
			...
			float div = 1.9;
			float valxa = length(color0)/div + sin(iTime*0.15)*0.1;
			
			if (valxa < 0.53) { color0 *= texture2D(u_texture1, v_texCoord); }
			else if (valxa < 0.58) { color0 *= texture2D(u_texture2, v_texCoord); }
			else if (valxa < 0.62) { color0 *= texture2D(u_texture3, v_texCoord); }
			else if (valxa < 0.7 + sin(iTime*0.5)*0.25) { color0 *= texture2D(u_texture4, v_texCoord); }
			else if (valxa < 0.75) { color0 *= texture2D(u_texture5, v_texCoord); }
			else if (valxa < 0.8 + sin(iTime*0.25)*0.25) { color0 *= texture2D(u_texture6, v_texCoord); }
			else if (valxa < 0.85 + cos(iTime*0.25)*0.5) { color0 *= texture2D(u_texture7, v_texCoord); }
			else { color0 *= texture2D(u_texture8, v_texCoord); }
	...
	gl.shaderSource(fragShader, fragCode4);
